...
class SimpleSerialTemplate:
    ...
    * hw_in_waiting()
    ...
    def read_bytes(self, num=0, timeout=250) -> bytes:
        """
        ...
        # Check number of bytes waiting in UART buffer (200 bytes)
        # -> detects buffer overruns and issues warning
        self.hardware_inWaiting()

        # Try to read from queue
        ret = bytearray()
        while num > 0 and self.target_count > 0:
            ret.append(self.target_queue.popleft())
            self.target_count -= 1
            num -= 1
        ...
        data = bytearray(self.hardware_read(num, timeout=timeout))
        for c in data:
    ...
    def read(self, num=0, timeout=250) -> str:
        return self.read_bytes(self, num, timeout).decode('latin-1')

    def peek_bytes(self, num=1, timeout=250) -> bytes:
        """
        Peek at data that would be returned by read_bytes() without consuming it.
        Reads from hardware if needed and appends to target_queue. Does not read more
        than space is available in the target_queue to prevent overflow.

        Args:
            num: The number of bytes to peek at. If 0, peek at all available data.
            timeout: How long to wait for hardware data, in ms.

        Returns:
            bytearray of peeked data (possibly shorter than num characters)
        """
        if num > self.max_queue_size:
            raise ValueError(f"Can only peek a maximum of {self.max_queue_size} bytes!")

        # First peek at what's already in target_queue
        peeked_data = bytearray()
        peek_count = min(num, self.target_count)

        # Peek at target_queue without popping
        for i, item in enumerate(self.target_queue):
            if i >= peek_count:
                break
            peeked_data.append(item)

        # If we need more data, read from hardware and append to target_queue
        if num > 0 and len(peeked_data) < num:
            remaining = num - len(peeked_data)

            # Read from hardware
            hardware_data = bytearray(self.hardware_read(remaining, timeout=timeout))

            # Append hardware data to target_queue
            for c in hardware_data:
                self.target_queue.append(c)

                # Also add to terminal_queue for logging (matching read() behavior)
                self.terminal_queue.append(['in', c])
                if self.terminal_count < self.max_queue_size:
                    self.terminal_count += 1
                else:
            ...
            # Add hardware data to peek result
            peeked_data.extend(hardware_data)

        return bytes(peeked_data)

    def peek(self, num=0, timeout=250) -> str:
        return self.peek_bytes(self, num, timeout).decode('latin-1')
    ...
    def terminal_read(self, num=0, timeout=250):
        ...
        for c in data:
            ...
            if self.target_count < self.max_queue_size:
                ...
                # TODO: This is bad
                # when using terminal inteface, data that has not been actually read (only by terminal) is overwrittenin the target_buffer
                # -> Do not use terminal read, it may be unreliable!!
